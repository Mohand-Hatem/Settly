...
| B3 | Email verification | Better Auth link **and** a custom 6-digit OTP | V1/O1/O3/O5/Y2 guarded by email verification (#38); mechanism = email links (#9) | **IMPLEMENTED (2026-09-19)** — link only (#9, #106); custom OTP removed | `identity/auth.ts`, `routes/index.ts` | — |
| B4 | Session policy | 30 d sliding, no absolute cap | 7 d sliding + 30 d absolute cap (V12) | **IMPLEMENTED (2026-09-19)** — 7 d sliding / 30 d cap (V12, #106) | `auth.ts` session config | — |
